...
export function normalizeSiteSettings(settings?: Partial<SiteSettings> | null): SiteSettings {
  ...
  return {
    ...
    registration_limit: Number.isFinite(Number(settings?.registration_limit)) ? Number(settings?.registration_limit) : null,
...
export function siteSettingsPayload(settings: SiteSettings) {
  return {
    ...
    registration_limit: settings.registration_limit ?? null,
